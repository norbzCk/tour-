...
function GuideDashboard() {
  ...
  const stats = [
    ...
    { icon: "⭐", title: "Rating", value: currentGuide?.rating || "4.5", color: "bg-amber-50 text-amber-700 dark:bg-amber-900/50 dark:text-amber-300" },
  ];
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className={`text-4xl font-bold mb-4 ${isDark ? "text-white" : "text-gray-900"}`}>Guide Profile Not Found</h1>
          <p className={isDark ? "text-gray-400" : "text-gray-600"}>Your guide account is not linked. Please contact admin.</p>
        </div>
      </div>
    );
  }
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
      ...
      <div className={`rounded-2xl shadow-sm border p-6 ${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"}`}>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <h3 className={`text-lg font-bold ${isDark ? "text-white" : "text-gray-800"}`}>My Assigned Tours</h3>
        ...
        {myTours.length === 0 ? (
          <div className="text-center py-12">
            <p className={`text-lg ${isDark ? "text-gray-400" : "text-gray-500"}`}>No tours assigned yet.</p>
        ...
        <h3 className={`text-lg font-bold mb-4 ${isDark ? "text-white" : "text-gray-800"}`}>Tour Bookings</h3>
        {filteredBookings.length === 0 ? (
          <p className={`text-center py-8 ${isDark ? "text-gray-400" : "text-gray-500"}`}>No bookings found for your tours.</p>
        ) : (
